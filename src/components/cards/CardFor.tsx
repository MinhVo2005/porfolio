...
import {
  ...
  CardNeofetch, CardHelp, CardError, CardSetup, CardTheme, CardCv,
...
export type CardResult = { title: string; card: React.ReactNode } | null;
...
export function CardFor(
  ...
  switch (cmd.toLowerCase()) {
    ...
    case "open": case "cat": {
      ...
      const projectName = arg.replace("--show", "").trim() || undefined;
      return { title: `${cmd} ${projectName ?? ""}`.trim(), card: <CardOpen user={user} name={projectName} showImg={showImg} /> };
    }
    ...
    case "cv":
      setActiveTab("cv");
      if (user.cvUrl) window.open(user.cvUrl, "_blank", "noreferrer");
      return { title: "cv", card: <CardCv cvUrl={user.cvUrl} /> };
